...
from aiogram import Bot, executor, Dispatcher, types
from scripts.markup_janre import Markup_janre
...
@dp.message_handler(commands=['janre'])
#функция куда попадает сообщение нашего пользователя types.Message - наше сообщение
async def janre_start(message: types.Message):
    #то что бот будет отвечать на /start
    #любой код
    print(123)
    await message.answer("Вот список жанров", reply_markup=Markup_janre.get_markup())
    #любой код
    await AwaitMessages.janre.set()



@dp.callback_query_handler(state=AwaitMessages.janre) 
async def janre(call:types.CallbackQuery, state: FSMContext):
    async with state.proxy() as data:
      await call.message.edit_text(f"Вы выбрали '{call.data}' как жанр")
      await state.finish()
